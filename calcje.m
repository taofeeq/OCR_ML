function [cnet,gradient] = calcje(cnet,e)
...
%cnet.Perf = mse (mse mean squared error between X and Y)
cnet.FLayer{k}.dEdX{1} = feval(mse, 'dy' , e, cnet.FLayer{k}.Y, cnet.FLayer{k}.X, mse);
...
%kron to obtain matrices product 
...
%Reshape data into single-column vector
 gradient=cnet.FLayer{k}.dEdW{1};
 gradient =[gradient;cnet.FLayer{k}.dEdB{1}];
...
if (cnet.numFLayers>1) %If there are more than 1 fully-connected layers
    for k=cnet.numLayers-1:cnet.numLayers-cnet.numFLayers+1
		...
		% converts cell array to an array 
       ...
       %Reshape data into single-column vector
         gradient =[gradient;cnet.FLayer{k}.dEdW{1}];
         gradient =[gradient;cnet.FLayer{k}.dEdB{1}];    
    end
end
...
for k=(cnet.numLayers-cnet.numFLayers):-1:2 %Exclude first layer from loop (it's dummy)
    ...
    %Reshape data into single-column vector
     gradient=[gradient;cell2mat(cnet.SLayer{k}.dEdW')];
     gradient=[gradient;cell2mat(cnet.SLayer{k}.dEdB')];
...
%---------------------------------------------------------------------------------------------    
     ...
     %Reshape data into single-column vector
     gradient=[gradient;reshape(cell2mat(cnet.CLayer{k}.dEdW),[],1)];
     gradient=[gradient;cell2mat(cnet.CLayer{k}.dEdB)'];
